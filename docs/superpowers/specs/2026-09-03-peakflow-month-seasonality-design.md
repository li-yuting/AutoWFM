...
1. **回测对比**：`backtest_sigma` 改动前后，M2-M3 / M3+ 的 MAPE 应下降；其余类型与整体不劣化。
   > 实测补充（2026-09-03）：`backtest_sigma` 的 14 天窗口全落在月末，测不到月初收益，改动后该窗口 MAPE 微升；改用**全月回测**（整个 8 月，同口径逐日 MAPE）后 M2-M3/M3+ 净改善或持平，月初 1-10 号显著下降（2~8.5pp）。故成功标准修正为「全月回测净改善/持平 + 月初显著改善」，σ 仍按 14 天窗口计算。
