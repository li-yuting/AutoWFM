...
实测结果（2026-09-03，改动前后对比）：

- **14 天回测窗口**（08-20~09-02，全落在月末）：M2-M3/M3+ 的 MAPE 微升（在线 M2-M3 16.77→17.84、M3+ 14.15→15.86；热线 M2-M3 26.86→26.91、M3+ 19.60→22.33）。该窗口全部是月末，测不到月初收益，属评估口径问题。
- **全月回测**（整个 8 月，同口径逐日 MAPE）：目标类型净改善或持平。

| 类型/渠道 | 全月(关→开) | 月初 1-10 号(关→开) |
|---|---|---|
| 在线 M2-M3 | 40.9%→39.2% | 47.6%→41.7% |
| 在线 M3+ | 24.7%→23.4% | 27.2%→24.1% |
| 热线 M2-M3 | 46.1%→43.3% | 46.0%→37.5% |
| 热线 M3+ | 32.0%→31.9% | 32.3%→30.0% |

- 其余类型 MAPE 不变（`use_month_seasonal=False`，路径不变）。

**验证口径（用户已确认方案 A）**：以「全月回测净改善/持平 + 月初显著改善」为成功标准；`backtest_sigma` 的 σ 仍按 14 天窗口计算（保守，三档区间不受影响）。
