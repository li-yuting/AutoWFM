...
由 AutoWFM 管理器(manager.py)作为受管子进程运行,替代直接运行排班 Web 应用:
- 将本仓库的 shift/ 目录加入 sys.path,以便其内部 `from reader import ...` 等导入可用;
...
SHIFT_DIR = Path(__file__).resolve().parent / "shift"
...
# 以 __main__ 身份执行 shift/app.py,触发 flask app.run
